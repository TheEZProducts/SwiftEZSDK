...
Модуль `EZUIPackKit` предоставляет единую систему переходов для управления навигацией между экранами. Точка входа — свойство **`ezTransit`** у `UIViewController` или `EZContainerView`.
...
В EZUIPackKit доступны встроенные анимации с `ez`-префиксом. Подробнее см. [EZTransitionAnimations](EZTransitionAnimations/README.md).
