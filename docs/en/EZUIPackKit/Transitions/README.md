...
The `EZUIPackKit` module provides a unified transition system for managing navigation between screens. The entry point is the **`ezTransit`** property on `UIViewController` or `EZContainerView`.
...
EZUIPackKit includes built-in animations with the `ez` prefix. For details, see [EZTransitionAnimations](EZTransitionAnimations/README.md).
