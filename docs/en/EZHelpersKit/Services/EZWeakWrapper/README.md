...
- store a **weak reference** inside a struct;
- (optionally) pass the wrapper between tasks, provided the wrapped object is `Sendable`.
...
- `value` is read-only from the outside (`private(set)`).
- `EZWeakWrapper` conforms to `Sendable` only when `Value: Sendable`.
