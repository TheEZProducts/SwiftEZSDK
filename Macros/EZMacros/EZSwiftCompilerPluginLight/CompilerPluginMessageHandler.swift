...
public protocol MessageConnection {
    ...
    func waitForNextMessage<RX: Decodable>(_ type: RX.Type) throws -> (RX, Data)?
}
...
extension CompilerPluginMessageHandler {
    ...
    func waitForNextMessage() throws -> (HostToPluginMessage, Data)? {
    ...
    public func main() throws {
        while let message = try self.waitForNextMessage() {
            try handleMessage(message.0, data: message.1)
        }
    }
    
    /// Handles a single message received from the plugin host.
    fileprivate func handleMessage(_ message: HostToPluginMessage, data: Data) throws {
        switch message {
