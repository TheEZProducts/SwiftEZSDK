...
#if os(Windows)
@inline(__always) private func _dupFD(_ fd: Int32) -> Int32 { _dup(fd) }
@inline(__always) private func _closeFD(_ fd: Int32) -> Int32 { _close(fd) }
@inline(__always) private func _dup2FD(_ src: Int32, _ dst: Int32) -> Int32 { _dup2(src, dst) }

// Windows CRT standard descriptors.
private let _STDIN_FD: Int32 = 0
private let _STDOUT_FD: Int32 = 1
private let _STDERR_FD: Int32 = 2
#else
@inline(__always) private func _dupFD(_ fd: Int32) -> Int32 { dup(fd) }
...
extension CompilerPlugin {
    ...
    fileprivate static func internalError(_ message: String) -> Never {
        if let data = ("Internal Error: \(message)\n").data(using: .utf8) {
...
internal struct PluginHostConnection: MessageConnection {
    ...
    func waitForNextMessage<RX: Decodable>(_ ty: RX.Type) throws -> (RX, Data)? {
        ...
        let json = try JSONDecoder().decode(RX.self, from: payload)
        return (json, payload)
    }
