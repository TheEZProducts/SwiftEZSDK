...
///
/// `EZPackMappedAccess` is the foundation of the IMV access control system. It allows interactors
...
///
/// // Usage:
/// let access = EZPackMappedAccess(mediator, accessMap: MyAccessMap(
///     userName: \.userName,
///     score: \.score
/// ))
...
@MainActor
@dynamicMemberLookup
open class EZPackMappedAccess<Object: AnyObject, AccessMap> {
...
/// - Important: Accessing the mediator before `didInitialize()` is called will trigger a fatal error.
@MainActor
open class EZPackMediatorAccessV<
    Mediator: EZPackMediatorBaseProtocol, AccessMap
>: EZPackMappedAccess<Mediator, AccessMap> {
    private var container = EZPackMediatorContainer<Mediator>(
        errorMessage: "Use the access only after the `didInitialize()` was called."
